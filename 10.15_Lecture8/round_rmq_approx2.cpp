...
void buildRMQ_ric(vector<T> &rmq, vector<T> input, range r, int pos){
    ...
    buildRMQ_ric(rmq, input, {m, r.hi}, RIGHT(pos));
...
T lazy_findMin(vector<T> &rmq, vector<T> &lazy, range const& q, range segment, int pos){
    ...
    //partial overlap
    int m = (segment.lo + segment.hi+1)/2;
    return query_t.func(lazy_findMin(rmq, lazy, q, {segment.lo, m}, LEFT(pos)), lazy_findMin(rmq,lazy, q, {m, segment.hi}, RIGHT(pos)));
}
...
int main(){
    ...
    for(int i=N; i<Np; i++){
        input.push_back(INFTY);
    }
    
    vector<int64_t> rmq;
    rmq.resize(2*Np-1);
    
    buildRMQ_ric<int64_t>(rmq, input, {0, Np}, 0);
    
    vector<int64_t>().swap(input);
    
    vector<int64_t> lazy;
    lazy.resize(2*Np -1 , 0);
    ...
    for(int i=0; i<M; i++){
        ...
        if (input.size() == 2){
            ...
            if (input[0]<= input[1]){
                cout << lazy_findMin<int64_t>(rmq, lazy, {input[0], input[1]+1}, {0, Np}, 0)<< endl;
            } else {
                int m1 = lazy_findMin<int64_t>(rmq, lazy, {input[0], N}, {0, Np}, 0);
                int m2 = lazy_findMin<int64_t>(rmq, lazy, {0, input[1]+1}, {0, Np}, 0);
                cout << min (m1, m2) <<endl;
            }
        }
        else if (input.size() == 3){
            //segments are circular
            if (input[0]<= input[1]){
                lazy_update<int64_t>(rmq, lazy, {input[0], input[1]+1}, {0, Np}, input[2], 0);
            } else {
                lazy_update<int64_t>(rmq, lazy, {input[0], N}, {0, Np}, input[2], 0);
                lazy_update<int64_t>(rmq, lazy, {0, input[1]+1}, {0, Np}, input[2], 0);
            }
        }
    }
}
